/**********************************************************/
/* C functions for Multiple precision linear computation  */
/* Supporting double, dd_real, qd_real and MPFR/GMP       */
/*                                                        */
/* Copyright (c) 2016 Tomonori Kouya, All rights reserved */
/* Version 0.0: 2016-11-17 (Thu) First published          */
/**********************************************************/
#ifndef __LINEAR_TK_C_MP__
#define __LINEAR_TK_C_MP__
...
void set_test_d_linear_eq(double A[], double true_x[], double b[], int dim)
{
	...
	for(i = 0; i < dim; i++)
	{
		true_x[i] = (double)(i + 1);
		for(j = 0; j < dim; j++)
//			A[ZERO_INDEX(i, j, dim)] = 1.0 /(double)(i + j + 1);
...
int d_solve_LU_linear_eq(double x[], double LU[], double b[], int dim, int pivot[])
{
	...
	{
		for(j = (i + 1); j < dim; j++)
			x[j] -= LU[ZERO_INDEX(pivot[j], i, dim)] * x[i];
	}
...
void mpfr_mydotp(mpfr_t ret, mpfr_t x[], mpfr_t y[], int dim)
{
	...
	for(i = 0; i < dim; i++)
		mpfr_fma(ret, x[i], y[i], ret, _tk_default_rmode);
		//ret += x[i] * y[i];
...
int mpfr_conjugate_gradient(mpfr_t x[], mpfr_t A[], mpfr_t b[], int dim, mpfr_t rel_tol, mpfr_t abs_tol, int maxitimes)
{
	...
	mpfr_inits2(prec, alpha, beta, tmp_val, init_r_norm2, r_new_norm2, (mpfr_ptr)0);
